...
if( length(tree_node.right_constrain) > 0)  
  ...
  %display([num2str( XData(tree_node.dim, :) ), ' ', num2str( tree_node.right_constrain ), ' ', num2str( y .* ((XData(tree_node.dim, :) < tree_node.right_constrain)) )]);
end
if( length(tree_node.left_constrain) > 0)
  y = y .* ((XData(tree_node.dim, :) > tree_node.left_constrain));
  %display([num2str( XData(tree_node.dim, :) ), ' ',  num2str( tree_node.left_constrain ), ' ', num2str( y .* ((XData(tree_node.dim, :) < tree_node.left_constrain)) )]);
end