...
double CSPHypothesis::Predict(double * in_Sample)
{
  ...
  {
    y = y* ( m_vSignums[i] * in_Sample[m_vDims[i]] > m_vSignums[i] * m_vThresholds[i] );
    //cout<< in_Sample[m_vDims[i]] <<" "<<m_vThresholds[i]<<" "<< (  m_vSignums[i] * in_Sample[m_vDims[i]] > m_vSignums[i] * m_vThresholds[i]  )<<endl;
    //cout<<endl;
